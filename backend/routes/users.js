...
const bcrypt = require('bcryptjs'); // Esta é a linha 3 correta
const crypto = require('crypto');

// NOTA: A importação do 'authorizeRole' está comentada pois o middleware está no server.js.
// Em uma futura refatoração, poderíamos mover os middlewares para sua própria pasta.
// const { authorizeRole } = require('../middlewares/auth'); 

const router = express.Router();

// Função para ser exportada e receber a conexão com o banco (pool) e os middlewares
const userRoutes = (pool, authorizeRole) => {
    ...
    router.post('/', authorizeRole(['dev', 'developer', 'administrator', 'admin']), async (req, res) => {
        ...
        // O creatorId é pego do token do usuário autenticado que está fazendo a requisição
